...
public class LogServiceImpl implements LogService {
   ...
   @Override
   public LogResponse createLog(LogCreateRequest request) {

      UUID projectId = projectRepository.findBySdkKey(request.getProjectKey())
            .orElseThrow(() -> new EntityNotFoundException("유효하지 않은 프로젝트 키"))
            .getId();

      UUID visitorId = visitorRepository.findByVisitorKey(request.getVisitorKey())
      ...
      return LogResponse.builder().
            id(savedLog.getId())
            .id(savedLog.getProjectId())
            .eventType(savedLog.getEventType())
